...
// Calculates the last stage of a type 2 sine transform when assuming the last stage is a factor 2
inline void _discrete_sine_type_2_factor_2_final_stage(const std::span<const float> input_sin, const std::span<const float> input_cos,
                                                      const std::span<float> output_sin)
{
    auto trf_size = input_sin.size();
    auto half_size = trf_size / 2;

    // Special handling of k == size - 1
    auto cos_even_k0 = input_cos[0];
    auto cos_odd_k0  = input_cos[1];

    output_sin[trf_size - 1] = cos_even_k0 - cos_odd_k0;
    ...
    for (uint32_t k = 1; k < half_size; k++)
    {
        ...
        auto cos_even = input_cos[2 * k];
        auto cos_odd  = input_cos[2 * k + 1];
        auto sin_even = input_sin[2 * (k - 1)];
        auto sin_odd  = input_sin[2 * (k - 1) + 1];

        auto cos_diff = cos_even - cos_odd;
        auto sin_sum  = sin_even + sin_odd;

        output_sin[k-1] = sin_sum * twiddle_cos - cos_diff * twiddle_sin;

        // Input block has half the size of output block (size N)
        // Extract values at N/2 + k by "reflecting" indices across N/2
        output_sin[trf_size - k - 1] = sin_sum * twiddle_sin + cos_diff * twiddle_cos;
    }

    // Separate handling of k = size // 2 because it reflects to itself
    auto sin_even_khalf = input_sin[2 * (half_size - 1)];
    auto sin_odd_khalf  = input_sin[2 * (half_size - 1) + 1];
    output_sin[half_size - 1] = (sin_even_khalf + sin_odd_khalf) * M_SQRT1_2;
}

// Calculates the last stage of a type 2 cosine transform when assuming the last stage is a factor 2
inline void _discrete_cosine_type_2_factor_2_final_stage(const std::span<const float> input_sin, const std::span<const float> input_cos,
                                                         const std::span<float> output_cos)
{
    ...
    output_cos[0] = cos_even_k0 + cos_odd_k0;
    ...
    for (uint32_t k = 1; k < half_size; k++)
    {
        ...
        output_cos[k] = cos_sum * twiddle_cos + sin_diff * twiddle_sin;

        // Input block has half the size of output block (size N)
        // Extract values at N/2 + k by "reflecting" indices across N/2
        output_cos[trf_size - k] = - cos_sum * twiddle_sin + sin_diff * twiddle_cos;
    }

    // Separate handling of k = size // 2 because it reflects to itself
    auto sin_even_khalf = input_sin[2 * (half_size - 1)];
    auto sin_odd_khalf  = input_sin[2 * (half_size - 1) + 1];
    output_cos[half_size] = (sin_even_khalf - sin_odd_khalf) * M_SQRT1_2;
}

// Calculates the last stage of a type 2 sine transform when assuming the last stage is a factor 3
inline void _discrete_sine_type_2_factor_3_final_stage(const std::span<const float> input_sin, const std::span<const float> input_cos,
                                                       const std::span<float> output_sin)
{
    auto trf_size = input_sin.size();
    ...
    for (uint32_t k = 1; k < third_size; k++)
    {
        ...
        auto cos_mod0 = input_cos[3 * k];
        auto cos_mod2 = input_cos[3 * k + 2];
...
inline void _discrete_cosine_type_2_factor_3_final_stage(const std::span<const float> input_sin, const std::span<const float> input_cos,
                                                         const std::span<float> output_cos)
{
    auto trf_size = input_cos.size();
    auto third_size = trf_size / 3;
...
inline void _discrete_cosine_type_3_factor_3(const std::span<const float> input_sin, const std::span<const float> input_cos,
                                             const std::span<float> output_sin, const std::span<float> output_cos,
                                             size_t stride, size_t block_size)
{
    const float sin_pi_over_3 = 0.5 * sqrtf(3.0f); // TODO replace with constexpr value
    ...
    for (size_t k = 0; k < third_size; k++)
    {
        ...
        // Note: the sin transform at these indices is never used, so we skip calculating them
        size_t block_idx = 0;
        auto sin_mod1 = input_sin[block_idx + stride * (3 * k + 1)];
        auto sin_mod2 = input_sin[block_idx + stride * (3 * k + 2)];
        auto cos_mod0 = input_cos[block_idx + stride * (3 * k)];
        ...
        float cos_term_1 = twiddle_cos        * cos_mod1 - twiddle_sin        * sin_mod1;
        float sin_term_1 = twiddle_sin        * cos_mod1 + twiddle_cos        * sin_mod1;
        float cos_term_2 = twiddle_cos_double * cos_mod2 - twiddle_sin_double * sin_mod2;
        float sin_term_2 = twiddle_sin_double * cos_mod2 + twiddle_cos_double * sin_mod2;

        float cos_refl_term_1 = (cos_term_1 + cos_term_2) * cos_pi_over_3;
        float cos_refl_term_2 = (sin_term_1 - sin_term_2) * sin_pi_over_3;

        output_cos[block_idx + stride * k] = cos_mod0 + cos_term_1 + cos_term_2;
        output_cos[block_idx + stride * (2 * third_size - k - 1)] = cos_mod0 - cos_refl_term_1 + cos_refl_term_2;
        output_cos[block_idx + stride * (2 * third_size + k)]     = cos_mod0 - cos_refl_term_1 - cos_refl_term_2;
        ...
        for (size_t block_idx = 1; block_idx < stride; block_idx++)
        {
            ...
            output_cos[block_idx + stride * k] = cos_mod0 + cos_term_1 + cos_term_2;
            output_sin[block_idx + stride * k] = sin_mod0 + sin_term_1 + sin_term_2;
            ...
            output_sin[block_idx + stride * (2 * third_size - k - 1)] = - sin_mod0 + sin_refl_term_1 + sin_refl_term_2;
            output_sin[block_idx + stride * (2 * third_size + k)]     =   sin_mod0 - sin_refl_term_1 + sin_refl_term_2;
            output_cos[block_idx + stride * (2 * third_size - k - 1)] =   cos_mod0 - cos_refl_term_1 + cos_refl_term_2;
            output_cos[block_idx + stride * (2 * third_size + k)]     =   cos_mod0 - cos_refl_term_1 - cos_refl_term_2;
        }
    }
...
std::vector<float> _discrete_cosine_transform_pow_2_type_2(const std::span<const float> input)
{
    ...
    _discrete_cosine_type_2_factor_2_final_stage(stage_input_sin, stage_input_cos, output);
    return output;
}
...
std::vector<float> _discrete_sine_transform_pow_2_type_2(const std::span<const float> input)
{
    ...
    const auto [stage_input_sin, stage_input_cos] = _discrete_sine_and_cosine_pow_2_type_2_common_stages(input);
    std::vector<float> output(input.size());
    _discrete_sine_type_2_factor_2_final_stage(stage_input_sin, stage_input_cos, output);
    return output;
}
...
std::vector<float> _discrete_cosine_transform_multi_radix_type_2(const std::span<const float> input, const std::span<const primes::prime_factor<size_t>> factors)
{
    ...
    // do final stage separately to avoid calculating an unnecessary sine step
    if (final_factor == 2)
    {
        _discrete_cosine_type_2_factor_2_final_stage(stage_input_sin, stage_input_cos, stage_output_cos);
    }
    else if (final_factor == 3)
    {
        _discrete_cosine_type_2_factor_3_final_stage(stage_input_sin, stage_input_cos, stage_output_cos);
    }

    if (stage_output_cos.data() == buffer_1_cos.data())
    {
        return buffer_1_cos;
    }
    else // stage_output_cos.data() == buffer_2_cos.data()
    {
        return buffer_2_cos;
    }
}
...
std::vector<float> _discrete_cosine_transform_multi_radix_type_3(const std::span<const float> input, const std::span<const primes::prime_factor<size_t>> factors)
{
    size_t trf_size = input.size();
    size_t block_size = 1;
    size_t stride = trf_size;
    ...
    // Construct four compute buffers:
    // buffer_1_cos is initialized to equal `input`, except the first element is halved.
    // buffer_2_sin and buffer_2_cos are default initialized with the same size as input.
    // The computation writes back and forth between buffer_1_[sin|cos] and buffer_2_[sin|cos]
    std::vector<float> buffer_1_cos(input.begin(), input.end());
    buffer_1_cos[0] *= 0.5f;
    std::vector<float> buffer_1_sin(trf_size);
    std::vector<float> buffer_2_sin(trf_size), buffer_2_cos(trf_size);
    std::span<float> stage_input_sin(buffer_1_sin);
    std::span<float> stage_input_cos(buffer_1_cos);
    std::span<float> stage_output_sin(buffer_2_sin);
    ...
    for (size_t i = 0; i < pow_3; i++)
    {
        block_size *= 3;
        stride /= 3;
        _discrete_cosine_type_3_factor_3(stage_input_sin, stage_input_cos, stage_output_sin, stage_output_cos, stride, block_size);
        std::swap(stage_input_sin, stage_output_sin);
        std::swap(stage_input_cos, stage_output_cos);
    }
...
std::vector<float> discrete_cosine_transform(const std::span<const float> input, SIN_COS_TRF_TYPE type)
{
    ...
    {
        return _discrete_cosine_transform_multi_radix_type_3(input, factors);
    }
